...
digital and analog inputs.  The ADCs are read at 5Hz.
...
\begin{center}
    \begin{tabular}{c|l|p{0.6\textwidth}}
        ...
        run at 50\% duty cycle for one minute and then shut off. \\
        ...
        shut down immediately and the fan will run at 50\% duty cycle for one minute and then shut 
        off. \\
    \end{tabular}
\end{center}
...
fan PWM.  The closer the actual temperature is to the target temperature (use the ``tt'' to set and
get this), the more slowly the fan will spin.  Fans react slowly to changes in speed since they take
time to accelerate; this is why the control loop runs so slowly.
...
The state machine is implemented in \path{firmware/app/control.c} and \path{control.h}2 se
\end{description}
